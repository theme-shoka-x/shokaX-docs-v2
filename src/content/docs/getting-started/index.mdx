...
针对 0.4.6 - 0.5，这是一个实验性特性，可能高度不稳定或无法正常运行
</Aside>


#### 使用 ShokaX-CLI 安装 ShokaX
<Aside type='caution'>
根据 ShokaX 生命周期计划，ShokaX-CLI 将于不晚于 2024 年 9 月正式弃用，新用户请参考上文的包管理器或初始化示例安装
</Aside>
...
** git 方式必须安装 pnpm **，不需要使用 pnpm 作为包管理器，但需要全局安装并添加到 path
</Aside>
